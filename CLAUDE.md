...
In every active HTML page (37 as of 2026-09-08), in 3 nav locations per file — desktop Dealers dropdown, footer, and the phone drawer generated by `mobile_menu.py` — this link appears immediately after the existing `Dealer Portal` link:
...
To find every occurrence: `grep -c 'walton-parts-catalog.vercel.app' *.html */*.html` — should return 3 per active page (desktop dropdown, footer, phone drawer). The drawer copy comes from `MENU` in `mobile_menu.py`; change it there and re-run the script rather than editing pages.
...
```bash
# Should output exactly 3 per active page (desktop dropdown + footer + phone drawer)
grep -c 'walton-parts-catalog.vercel.app' *.html */*.html | grep -v '_archive\|Product Page\|404.html'

# The phone drawer copy is generated — confirm no page has drifted from mobile_menu.py
python3 mobile_menu.py --check
...
**Maintained scripts (not one-shots — keep them):**

- `mobile_menu.py` — single source of truth for the phone menu drawer. The drawer markup is stamped into every page; edit `MENU` in the script and re-run it. `--check` reports drift (CI-friendly).
- `optimize_images.py` — writes 800/1200/1600 px WebP derivatives for every referenced JPG/PNG and rewrites `<img>` tags with `srcset`/`sizes`/`loading="lazy"`. Run it after adding a photo; commit the `.webp` files with the page.

**One-shot helper (safe to delete):**
